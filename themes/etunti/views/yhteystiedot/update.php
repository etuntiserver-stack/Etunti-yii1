...
if(isset($_POST['uploaded_t']))
{
  ...
  if (!file_exists(Yii::app()->basePath."/../tiedostot/yhteystiedot/".Yii::app()->user->domain)) {
  	mkdir(Yii::app()->basePath."/../tiedostot/yhteystiedot/".Yii::app()->user->domain, 0777, true);
  }

  $uploaddir = Yii::app()->basePath.'/../tiedostot/yhteystiedot/'.Yii::app()->user->domain.'/';
...
<br>
	...
	$i = 0;
	foreach(array_reverse(glob('tiedostot/yhteystiedot/'.Yii::app()->user->domain.'/'.$model->id.'_*.*')) as $file) {
	$i++;
