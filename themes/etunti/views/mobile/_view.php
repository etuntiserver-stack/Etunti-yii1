...
/* TAG */
 ...
 if($pv && count($pv) > 0 && $viesti && strlen($viesti) > 0) {
	$pikkuviesti = '<div class="row"><div class="col-sm-12 text-danger"><strong>'.Yii::t('main', 'Viesti').':</strong><br><p>';
	foreach($pv as $part) {
		$pikkuviesti .= $part . "<br>";
	}
	
	// remove last <br>
	$pikkuviesti = substr($pikkuviesti, 0, -4);
	$pikkuviesti .= '</p></div></div>';
 }
...
<tr style="<?php echo $class; ?>" id="rivi_<?php echo $data->id; ?>">
	...
	<td class="text-center">
		...
		<tr>
		<td style="padding: 3px 5px" width="50%"><span data-toggle="tooltip" data-placement="top" title="Aloitus karttalla"><?=$karttaA?></span></td>
		<td style="padding: 3px 5px" width="50%"><span data-toggle="tooltip" data-placement="top" title="Lopetus karttalla"><?=$karttaL?></span></td>
		</tr>
		<?php 
		if(
			$data->tv_id > 0 
