        ...
        $('#korvaukset-form').on('submit', function(e) {
            ...
            $.ajax({
                ...
                success: function(data) {
                    //console.log(data);
                    if (data == 'ok') {
                        // this block will never even get executed, but kotipuhtaaksi we wouldn't want that anyways.
                        setTimeout(function() {
                            document.location.href = "palkkataulukko";
                        }, 500);
                        $("#haku").collapse("hide");
                    } else if (data === '"ok"') {
                        // when on success we'll clear and hide the form (kind of like it's working now
                        // except you can actually open the form a second time after submitting... unlike now)
                        // clear form
                        $("#Korvaukset_tid").select2().val(null).trigger("change");
                        $("#Korvaukset_syy").val("");
                        $("#Korvaukset_korvaus").val("");
                        // collapse form
                        $("#haku").collapse("hide");
                    } else {
