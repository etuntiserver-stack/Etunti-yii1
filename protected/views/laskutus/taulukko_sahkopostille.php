<?php
// taulukko_sahkopostille.php
?>

<h2>Odottava lähetys SÄHKÖPOSTI</h2>
<div>
...
<?php
   ...
   $lasku = Laskutus::model()->findAll(" tilanne = 'avoin' AND laskutus = 'sahkoposti' ");
   foreach($lasku as $l)
   {
   if($l->nimi)
	$nimi = $l->nimi;
   ...
   if($l->erapaiva < date("Y-m-d") and $l->tilanne == 'lähetetty')
   $erapaiva = '<span class="erapaiva">'.date("d.m",strtotime($l->erapaiva)).'</span>';
   else
   $erapaiva = '<span>'.date("d.m",strtotime($l->erapaiva)).'</span>';
