	...
	*/
	...
	public function TidfromtoMobiili($from, $to, $tid, $status=array(), $hyvaksytty='', $ilta=null, $yo=null, $su=null)
	{
		...
		$ilta_criteria = "
        	SUM(CASE 
	            WHEN 
			TIME(DATE_FORMAT(STR_TO_DATE(aloitan, '%d.%m.%Y %H:%i'), '%H:%i:%s')) >= TIME('18:00:00') 
			&& TIME(DATE_FORMAT(STR_TO_DATE(loppui, '%d.%m.%Y %H:%i'), '%H:%i:%s')) > TIME('18:00:00') 
			&& TIME(DATE_FORMAT(STR_TO_DATE(loppui, '%d.%m.%Y %H:%i'), '%H:%i:%s')) <= TIME('23:00:00') 
			   THEN TIME_TO_SEC(TIMEDIFF(DATE_FORMAT(STR_TO_DATE(loppui, '%d.%m.%Y %H:%i'), '%H:%i:%s'), DATE_FORMAT(STR_TO_DATE(aloitan, '%d.%m.%Y %H:%i'), '%H:%i:%s')))
	            WHEN 
			TIME(DATE_FORMAT(STR_TO_DATE(aloitan, '%d.%m.%Y %H:%i'), '%H:%i:%s')) < TIME('18:00:00') 
			&& TIME(DATE_FORMAT(STR_TO_DATE(loppui, '%d.%m.%Y %H:%i'), '%H:%i:%s')) > TIME('18:00:00') 
			&& TIME(DATE_FORMAT(STR_TO_DATE(loppui, '%d.%m.%Y %H:%i'), '%H:%i:%s')) <= TIME('23:00:00') 
			   THEN TIME_TO_SEC(TIMEDIFF(DATE_FORMAT(STR_TO_DATE(loppui, '%d.%m.%Y %H:%i'), '%H:%i:%s'), TIME('18:00:00')))
	            WHEN 
			TIME(DATE_FORMAT(STR_TO_DATE(aloitan, '%d.%m.%Y %H:%i'), '%H:%i:%s')) >= TIME('18:00:00') 
			&& 
			(
			  ...
			  || TIME(DATE_FORMAT(STR_TO_DATE(loppui, '%d.%m.%Y %H:%i'), '%d.%m.%Y')) != TIME(DATE_FORMAT(STR_TO_DATE(aloitan, '%d.%m.%Y %H:%i'), '%d.%m.%Y'))
			   ...
			   THEN TIME_TO_SEC(TIMEDIFF(TIME('23:00:00'), DATE_FORMAT(STR_TO_DATE(aloitan, '%d.%m.%Y %H:%i'), '%H:%i:%s')))
	            WHEN 
			TIME(DATE_FORMAT(STR_TO_DATE(aloitan, '%d.%m.%Y %H:%i'), '%H:%i:%s')) < TIME('18:00:00') 
			&& 
			(
			  ...
			  || TIME(DATE_FORMAT(STR_TO_DATE(loppui, '%d.%m.%Y %H:%i'), '%d.%m.%Y')) != TIME(DATE_FORMAT(STR_TO_DATE(aloitan, '%d.%m.%Y %H:%i'), '%d.%m.%Y'))
			   ...
			   THEN TIME_TO_SEC(TIMEDIFF(TIME('23:00:00'), TIME('18:00:00')))
	            ELSE 0
	        END) AS l_tunnit
		";
		//     ILTA -->
