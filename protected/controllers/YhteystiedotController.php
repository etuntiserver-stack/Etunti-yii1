...
class YhteystiedotController extends Controller
{
	...
	// <-- Oikeudet
	   $checkOikeus = "yhteystiedot_1_".Yii::app()->user->adminStatus;
	   $site = Yii::app()->createController('Site');
	   $site[0]->checkOikeus($checkOikeus);
	//  Oikeudet -->

		$model=new Yhteystiedot;

		// Uncomment the following line if AJAX validation is needed
		$this->performAjaxValidation($model);

		if(isset($_POST['Yhteystiedot']))
		{
	...
	// <-- Oikeudet
	   $checkOikeus = "yhteystiedot_2_".Yii::app()->user->adminStatus;
	   $site = Yii::app()->createController('Site');
	   $site[0]->checkOikeus($checkOikeus);
	//  Oikeudet -->
	...
	// <-- Oikeudet
	   $checkOikeus = "yhteystiedot_3_".Yii::app()->user->adminStatus;
	   $site = Yii::app()->createController('Site');
	   $site[0]->checkOikeus($checkOikeus);
	//  Oikeudet -->
	...
	// <-- Oikeudet
	   $checkOikeus = "yhteystiedot_0_".Yii::app()->user->adminStatus;
	   $site = Yii::app()->createController('Site');
	   $site[0]->checkOikeus($checkOikeus);
	//  Oikeudet -->
		...
		$dataProvider=new CActiveDataProvider('Yhteystiedot', array(
			'criteria'=>$criteria,
			//'pagination'=>false
		));
	...
	{
		$model=new Yhteystiedot('search');
		$model->unsetAttributes();  // clear any default values
		if(isset($_GET['Yhteystiedot']))
			$model->attributes=$_GET['Yhteystiedot'];
	 ...
	 * @return Yhteystiedot the loaded model
	 * @throws CHttpException
	 */
	public function loadModel($id)
	{
		$model=Yhteystiedot::model()->findByPk($id);
		if($model===null)
	 ...
	 * @param Yhteystiedot $model the model to be validated
	 */
	protected function performAjaxValidation($model)
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='yhteystiedot-form')
		{
	...
	{ 
		$return = '';
		    ...
		    $a = Yhteystiedot::model()->findbypk($as);
		    ...
		    if(isset($a->yhteystieto_tyyppi) and !empty($a->yhteystieto_tyyppi) and $a->yhteystieto_tyyppi == 'henkilo')
		    $return = '<b class="text-warning">Yhteyshenkilö</b><br>'.$return;
		    elseif(isset($a->yhteystieto_tyyppi) and !empty($a->yhteystieto_tyyppi) and $a->yhteystieto_tyyppi == 'yritys')
