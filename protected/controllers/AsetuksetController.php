...
Jos yritykselläsi ei ole Ropo Capital Oy:n kanssa sopimusta tunnuksista, lähetä viesti osoitteeseen tuki@etunti.fi ja autamme sopimuksen syntymisessä.");
	...
	{
		// Settings.
		$client_id = urlencode('etuntiTestClient');
		$client_secret = urlencode('testsecret_W2ir6fiE4fdtO3Htevx9');
		$redirect_uri = urlencode('redirect-uri-placeholder');
		$user = urlencode('etunti.test');
		$pw = urlencode('Elias2011!');
		$company = urlencode(15022);
		...
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_URL, "https://api-test.procountor.com/api/oauth/authz?response_type=code&client_id=$client_id");
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, "response_type=code&username=$user&password=$pw&company=$company&redirect_uri=$redirect_uri");
		...
		curl_setopt($ch, CURLOPT_POST, 1);
		curl_setopt($ch, CURLOPT_POSTFIELDS, "client_id=$client_id&client_secret=$client_secret&grant_type=authorization_code&redirect_uri=$redirect_uri&code=$code");
		curl_setopt($ch, CURLOPT_HTTPHEADER, ["Content-Type: application/x-www-form-urlencoded"]);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
